...
async function getAdvices(profession) {
  try {
    const response = await fetch(`https://34.107.29.191:80/api/advices?profession=${profession}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Origin': window.location.origin
...
async function handleSubmitAddressInfo(event) {
  ...
  async function sendQuoteRequest(data) {
    try {
      const response = await fetch('https://34.107.29.191:80/api/quotes', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Origin': window.location.origin
...
function newQuote() {
  ...
  document.getElementById('FormProfession').addEventListener('submit', handleSubmitProfessionInfo);
  if (token) {
    fetch(`https://34.107.29.191:80/api/quotes/archive?token=${token}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        'Origin': window.location.origin
    ...
    fetch(`https://34.107.29.191:80/api/quotes/archive?ip_address=${ip}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        'Origin': window.location.origin
